 ...
 * 校长致辞
 */
public class SubjectBuildAction extends ActionSupport {
    //注入元素
    ...
    //查找通知
    public String findBuild() throws Exception {
        ...
        if (titleList.size() > 0) {
            return "findBuildSubject";
        } else {
            return "addBuildSubject";
        }
    }

    //查找一个通知
    public String findOneBuild() throws Exception {
        if (tid == null || tid == 0) {
            return ERROR;
        }
        title = titleService.findTitleById(tid);
        return "updateOneBuildSubject";
    }
    ...
    public String addBuild() throws Exception {
        ...
        title.setTRead(0);
        Category category = categoryService.getCategoryByName("学科建设");
    ...
    //删除通知
    public String deleteBuild() throws Exception {
