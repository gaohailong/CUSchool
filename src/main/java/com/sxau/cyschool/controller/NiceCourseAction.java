 ...
 *
 */
public class NiceCourseAction extends ActionSupport {
    //注入元素
    ...
    public String findNice() throws Exception {
        ...
        return "findNiceCourse";
    }

    //查找一个现任领导
    public String findOneNice() throws Exception {
        if (tid == null || tid == 0) {
            return ERROR;
        }
        title = titleService.findTitleById(tid);
        return "updateNiceCourse";
    }

    //添加现任领导通知
    public String addNice() throws Exception {
        Title title = new Title();
        title.setTContent(notificationContent);
        title.setTName(notificationHead);
        title.setTDate(new Date());
        title.setTRead(0);
        Category category = categoryService.getCategoryByName("精品课程");
    ...
    //删除现任领导通知
    public String deleteNice() throws Exception {
        titleService.deleteTitle(tid);
        return SUCCESS;
    }

    //修改现任领导
    public String updateNice() throws Exception {
        if (tid == null || tid == 0) {
            return ERROR;
        }
        title = titleService.findTitleById(tid);
