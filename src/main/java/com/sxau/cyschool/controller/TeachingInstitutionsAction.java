 ...
 */
public class TeachingInstitutionsAction extends ActionSupport {
    //注入元素
    ...
    //查找一个通知
    public String findOneInstitutions() throws Exception {
        if (tid == null || tid == 0) {
            return ERROR;
        }
        title = titleService.findTitleById(tid);
        return "findOneInstitutions";
    }

    //添加通知
    public String addInstitutions() throws Exception {
        ...
        title.setTRead(0);
        Category category = categoryService.getCategoryByName("教辅机构");
    ...
    //删除通知
    public String deleteInstitutions() throws Exception {
        titleService.deleteTitle(tid);
        return SUCCESS;
    }

    //修改通知
    public String updateInstitutions() throws Exception {
        if (tid == null || tid == 0) {
            return ERROR;
        }
        title = titleService.findTitleById(tid);
